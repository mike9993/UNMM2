...
-- Farm Speed Dial Card (redesigned)
...
-- Exceed speed toggle inside farm speed card
...
-- Anti-Die Section (collapsible)
local antiKillSection = makeSection("Anti-Die", farmContainer, "Farm", false, false)

partState["antiKill"] = cfgAntiKill
createTogglePart(antiKillSection.Content, "Anti-Die", "antiKill", 1, false, function(state)
cfgAntiKill = state
triggerAutoSave()
if state then
startAntiKill()
...
-- Detection Radius Dial Card (same dial style as farm speed)
...
-- Anti-Die: invis when murderer is within radius
...
if (obj.Position - hrpPosition).Magnitude < 15 then
return true
end
end
end
...
local nearMurderer = false
for _, player in ipairs(Players:GetPlayers()) do
if player ~= LocalPlayer then
local role = getActualRole(player)
if role == "Murderer" then
local mHRP = player.Character and player.Character:FindFirstChild("HumanoidRootPart")
if mHRP and (hrp.Position - mHRP.Position).Magnitude < cfgAntiKillRadius then
nearMurderer = true
break
end
end
...
if not nearMurderer then
nearMurderer = isKnifeNearby(hrp.Position)
end

if nearMurderer then
local _CFrame = hrp.CFrame
local _Offset = hum.CameraOffset
local farPos = _CFrame * CFrame.new(0, -200000, 0)
hrp.CFrame = farPos
hum.CameraOffset = farPos:ToObjectSpace(CFrame.new(_CFrame.Position)).Position
